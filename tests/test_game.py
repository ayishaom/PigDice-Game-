...
Covers human and AI turns, rolling, holding, cheating, name changes,
help commands,
...
class TestPigGame(unittest.TestCase):
    """Unit tests for the PigGame class covering I/O, turns, scoring, AI,
    and commands."""

    def setUp(self):
        """Set up two human players and a PigGame instance with mocked dice
        and AI agent."""
    ...
    def run_game_with_inputs(self, inputs):
        ...
        Args:
            inputs (list[str]): List of input strings to simulate
            user commands.
    ...
    def test_roll_then_hold_banks_points_and_switches_turn(self):
        """Test that rolling then holding banks points and ends
        turn properly."""
    ...
    def test_cheat_wins_and_records_game(self):
        """Test that using cheat adds points and triggers a win, recording
        the game."""
    ...
    def test_help_command_shows_text_then_quit(self):
        """Test that help command prints instructions and allows
        continued play."""
    ...
    def test_human_win_by_reaching_threshold_during_roll(self):
        """Test that a player wins immediately if roll reaches
        winning score."""
    ...
    def test_ai_can_win_by_rolling_and_records_game(self):
        """Test that AI can win by rolling without user input and game
        is recorded."""
        self.p1.is_ai = True
    ...
    def test_ai_change_cancel_when_empty(self):
        """Test that empty input during AI difficulty change cancels
        the operation."""
        self.p2.is_ai = True
    ...
    def test_init_raises_when_not_two_players(self):
        """Test that PigGame raises ValueError if not exactly two players
        are provided."""
        p1 = Player("A")
        score = Score()
    ...
    def test_prompt_cmd_keyboardinterrupt_returns_quit(self):
        """Test that _prompt_cmd handles KeyboardInterrupt and
        returns 'quit'."""
    ...
    def test_prompt_ai_level_eof_returns_none_and_prints_cancelled(self):
        """Test that _prompt_ai_level returns None on EOFError
        and prints cancellation."""
        g = PigGame([Player("A"), Player("B")], Score())
        out = io.StringIO()
        with (
    ...
    def test_roll_none_values_is_safe_and_shows_question_mark(self):
        """Test that DiceHand.roll() returning empty list is handled
        safely with '?'."""
        dice = MagicMock()
    ...
    def test_ai_level_invalid_then_valid_prints_warning(self):
        """Test that invalid AI difficulty input is rejected and
    ...
    def test_ai_change_when_no_ai_exists_shows_info(self):
        """Test that attempting AI change with no AI opponent
        shows info message."""
        self.p2.is_ai = False
        out = self.run_game_with_inputs(["ai", "q"])
    ...
    def test_name_change_keyboardinterrupt_cancel(self):
        """Test that KeyboardInterrupt during name change cancels
        the operation."""
        out = io.StringIO()
        with (
            patch(
                "builtins.input",
                side_effect=["name", KeyboardInterrupt, "q"],
            ),
            patch("sys.stdout", new=out),
        ):
            self.game.play()
    ...
    def test_name_change_no_previous_stats_message(self):
        """Test that renaming a player with no previous stats shows
